...
final class MigrationAdminController {
	...
	public static function handle_flow_prepare(): void {
		self::authorize( self::FLOW_PREPARE_ACTION );
	...
	private static function scope_request( string $action ): string {
		...
		$scope_id = isset( $_POST['scope_id'] ) ? sanitize_text_field( (string) wp_unslash( $_POST['scope_id'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Scope is required to derive the target-bound nonce.
		if ( '' === $scope_id ) {
			return '';
		}
	...
	private static function redirect( string $state ): void {
		$url = add_query_arg( array( 'page' => AdminDefinition::SETTINGS_SLUG, 'gnm_migration' => $state ), admin_url( 'admin.php' ) );
