...
final class GravityFlowAssigneeReaderTest extends TestCase {
	...
	/**
	 * Install the deterministic global Gravity Flow API alias.
	 *
	 * @return void
	 */
	...
	public function test_exact_selected_step_is_used_instead_of_current_notification_step(): void {
		...
		$result = ( new GravityFlowAssigneeReader() )->read(
			$entry,
			array(
				'id' => 9,
			),
	...
	public function test_selected_step_unavailable_states_fail_closed(): void {
		...
		GravityFlowExactStepApiStub::configure(
			array(
				83 => new GravityFlowAssigneeStepStub( 'not-an-array' ),
			),
			null
		);
		...
		GravityFlowExactStepApiStub::configure(
			array(
				85 => new GravityFlowAssigneeStepStub( array() ),
			),
			null
		);
