...
final class AdvisorAdminContractTest extends TestCase {
	...
	public function test_advisor_is_registered_and_reachable_from_plugin_entrypoint(): void {
		...
		self::assertMatchesRegularExpression( "/AdminDefinition::ADVISOR_SLUG\\s*=>\\s*'render_advisor'/", $controller );
	...
	public function test_advisor_render_path_is_side_effect_free(): void {
		...
		self::assertStringNotContainsString( 'ProviderConnectionService', $section );
		self::assertStringNotContainsString( 'ProviderDiscoveryService', $section );
		self::assertStringNotContainsString( 'gform_update_meta', $section );
		self::assertStringNotContainsString( 'update_option', $section );
		self::assertStringNotContainsString( 'wp_remote_', $section );
	...
	public function test_advisor_flow_links_reuse_existing_navigation_helper(): void {
		self::assertSame( 'admin.php?page=gf_edit_forms&view=settings&subview=gravityflow&id=7', GravityFlowNavigation::relative_path( 7, true ) );
	...
	/** SMS provider guidance navigates to Provider Manager without registering a new send endpoint. */
	public function test_advisor_provider_guidance_reuses_provider_manager_surface(): void {
		...
		self::assertStringContainsString( "'action'       => self::ACTION_PROVIDERS", $model );
		self::assertStringNotContainsString( 'ProviderTestService', $model );
		self::assertStringNotContainsString( 'admin_post_', $model );
	}

	/** Retry guidance preserves Entry Detail/manual Retry ownership. */
