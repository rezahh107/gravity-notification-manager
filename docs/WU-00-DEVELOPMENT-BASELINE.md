...
PHPStan remains a repository development dependency and now has one tracked root-level scope contract: `phpstan.neon.dist`. That file is the sole owner of the maintained production analysis boundary: `src/`, `includes/`, and `gravityflow-sms-ippanel.php`. The contract uses PHPStan level `0`, the minimum supported baseline, so establishing the missing scope does not independently raise static-analysis strictness. External WordPress / Gravity Forms / Gravity Flow symbols that are not installed in the ordinary qualification job are described only through the scan-only `phpstan-stubs.php` development file; that file is not part of the analysis path and is never loaded by the plugin runtime. `composer stan` consumes the configuration without duplicating path arguments, and WU-00 treats both configuration errors and genuine PHPStan findings as blocking nonzero qualification results.
